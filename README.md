...
- 環境変数: `.env` を作成し、ローカル用に `NUXT_OPENAI_API_KEY=` を設定（テンプレは `.env.example`）。Cloudflare では `wrangler secret put NUXT_OPENAI_API_KEY` でシークレット登録します。
...
- シークレット登録: `wrangler secret put NUXT_OPENAI_API_KEY`
- ビルド: `npm run build`
